...
def main():
    ...
    if options.favorites:
        ...
        if (options.is_download or options.is_iddownload) and doujinshis:
        ...
        print_doujinshi(doujinshis)
        if options.is_download or options.is_iddownload:
            doujinshi_ids = [i['id'] for i in doujinshis]
    ...
    if options.is_iddownload:
        with open(os.path.join(options.output_dir, 'doujinshiID.txt'), 'w') as f:
            for id_ in doujinshi_ids:
                f.write(str(id_))
                f.write('\n')
                logger.info('added doujinshi id %d to %s' % (int(id_), 'doujinshiID.txt'))
